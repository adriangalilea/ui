...
- **Commits are conventional: `type(scope): description`**, scope = the item
  (`fix(telegram-chat): …`, `docs: …`, `chore: …`). `.githooks/commit-msg` refuses
  anything else; it is wired once per clone with `git config core.hooksPath .githooks`.
  `mise changes` and the changelog review read the type; a capitalised sentence is
  invisible to both.
